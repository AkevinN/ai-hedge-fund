    ...
    *,
    ...
    if include_analyst_flags:
        parser.add_argument(
            ...
            type=str,
            required=False,
            help="要使用的分析师列表，用逗号分隔 (例如: michael_burry,other_analyst)",
        )
        parser.add_argument(
            "--analysts-all",
            action="store_true",
    ...
    parser.add_argument("--model", type=str, required=False, help="要使用的模型名称 (例如: gpt-4o)")
    return parser
...
def parse_cli_inputs(
    *,
    ...
    parser.add_argument(
        ...
        type=float,
        default=100000.0,
        help="初始资金 (别名: --initial-capital)，默认值: 100000.0",
    )
    parser.add_argument(
        "--margin-requirement",
